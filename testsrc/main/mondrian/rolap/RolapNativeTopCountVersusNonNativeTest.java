...
public class RolapNativeTopCountVersusNonNativeTest extends BatchTestCase {
    ...
    private void assertResultsAreEqual(String testCase, String query, TestContext ctx) {
        Result nativeResult = executeNative(query, ctx);
        ctx.flushSchemaCache();
        Result nonNativeResult = executeNonNative(query, ctx);
        assertEquals(
                String.format("[%s]: native and non-native results of the query differ. The query:\n\t\t%s", testCase, query),
    ...
    public void testTopCount_ImplicitCountMeasure() throws Exception {
        assertResultsAreEqual("Implicit Count Measure", IMPLICIT_COUNT_MEASURE_QUERY);
    }

    public void testTopCount_SumMeasure() throws Exception {
        assertResultsAreEqual("Sum Measure", SUM_MEASURE_QUERY);
    }
    ...
    public void testTopCount_CountMeasure() throws Exception {
        ...
        assertResultsAreEqual("Custom Count Measure", CUSTOM_COUNT_MEASURE_QUERY, ctx);
    }


    public void testEmptyCellsAreShown_Countries() throws Exception {
        assertResultsAreEqual("Empty Cells Are Shown - Countries", EMPTY_CELLS_ARE_SHOWN_COUNTRIES_QUERY);
    }

    public void testEmptyCellsAreShown_States() throws Exception {
        assertResultsAreEqual("Empty Cells Are Shown - States", EMPTY_CELLS_ARE_SHOWN_STATES_QUERY);
    }

    public void testEmptyCellsAreShown_ButNoMoreThanReallyExist() {
        assertResultsAreEqual("Empty Cells Are Shown - But no more than really exist", EMPTY_CELLS_ARE_SHOWN_NOT_MORE_THAN_EXIST_QUERY);
    }

    public void testEmptyCellsAreHidden_WhenNonEmptyIsDeclaredExplicitly() {
        assertResultsAreEqual("Empty Cells Are Hidden - When NON EMPTY is declared explicitly", EMPTY_CELLS_ARE_HIDDEN_WHEN_NON_EMPTY_QUERY);
    }
    ...
    public void testMimicsHeadWhenTwoParams_ShowsNotMoreThanExist() {
        assertResultsAreEqual("Two Parameters - Shows not more than really exist", RESULTS_ARE_SHOWN_NOT_MORE_THAN_EXIST_TWO_PARAMS_QUERY);
    }

    public void testNonEmptyIsIgnored_WhenTwoParams() {
        assertResultsAreEqual("Two Parameters - NON EMPTY is ignored", NON_EMPTY_IS_IGNORED_WHEN_TWO_PARAMS_QUERY);
    }
}
